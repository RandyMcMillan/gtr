// TODO: implement transport trait so that different transports can get
...
//     let router = SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(127, 0, 0,
// 1), 6882));
//
//     // TODO: make configurable
...
//
//     // TODO: use more sophisticated handshaker, see example from
// bip_handshake     //let handshaker = SimpleHandshaker{ filter:
// HashSet::new(), count: 0 };     let peer_id =
// (*b"-UT2060-000000000000").into(); // bootstrap peer_id???     let handshaker
...
// }
// // TODO:
// // 1. spin up DHT and make it listen for connections on on configurable
// <listen net port> // NOTE: bip-dht will immediately try to bootstrap so with
// provided bootstrap nodes // NOTE: for local setup bip-dht needs three nodes,
// as table will not be populated // NOTE: UPDATE: router is not included into
// nodes table, thus if DHT is started with node instead // of with router, two
// nodes should suffice //     by information from the initial node
// //
...
// // Sign value with generated keys (TODO: check when to do this and is nostr
// keys can/should be // reused)
// // Create opts object:
// // {
// //      k: 32bytes padded public key
// //      seq: sequnce number starting from 0 (NOTE: gittorrentd does not
// increment it and it does //          not seem to be needed)
// //      v: buffered serialized Repository object
// //      sig: Concatenation of 32bytes padded ed25519 signature r and s values
// (standard) // }
// //
// // 4. Create a net server and with web socket on <net announce port> and ut
// wire for torrent and metadata // NOTE: gittorrentd uses ut_gittorrent and
// ut_metadata. Their role might be partially or fully //      implemented in
// bip-dht, bip-metainfo and/or bip-utracker // Pipe server's socket into wire
