...
use tokio::{
    fs::{File, OpenOptions},
    io::{AsyncReadExt, AsyncWriteExt, BufReader, ErrorKind},
    process::{Child, ChildStdin, ChildStdout, Command},
};

use crate::utils::error::{GitError, GtrResult};
...
pub async fn gtr_setup(dir: &PathBuf) -> GtrResult<()> {
    if !is_git(dir) {
        return Err(GitError::not_git_repo(dir));
    };
...
pub async fn select_exsiting_branches(
    dir: &str,
    branches: &Vec<&String>,
) -> GtrResult<Vec<String>> {
...
/// Returns hash of Ref for each branch of given repository as well as current
/// HEAD
...
async fn read_pkt_line(buf: &mut BufReader<ChildStdout>) -> GtrResult<Option<Vec<u8>>> {
    ...
    buf.read_exact(&mut len_bytes)
        .await
        .map_err(|e| GitError::pack_read_failed(Box::new(e)))?;
    ...
    let len_str =
        str::from_utf8(&len_bytes).map_err(|e| GitError::pack_read_failed(Box::new(e)))?;
    let len =
        usize::from_str_radix(len_str, 16).map_err(|e| GitError::pack_read_failed(Box::new(e)))?;
...
async fn request_pack_file(
    ...
    want: &str,
    have: Option<&str>,
) -> GtrResult<Option<Vec<u8>>> // Now returns Option<Vec<u8>>
{
    ...
    loop {
        match read_pkt_line(buf).await? {
            Some(data) => {
                ...
                let line =
                    String::from_utf8(data).map_err(|e| GitError::pack_read_failed(Box::new(e)))?;

                if !expect_nack {
                    // Not expecting NAK yet, still capabilities
                    continue;
                }

                if let Some(_) = have {
                    ack_objects_continue(&line);
                } else {
                    wait_for_nak(&line);
                };
            }
            None => {
                // '0000' pkt-line received, end of capabilities/message
                if !expect_nack {
...
async fn write_pack_file(
    dir: &PathBuf,
    want: &str,
    initial_pack_data: Option<Vec<u8>>,
    buf: &mut BufReader<ChildStdout>,
) -> GtrResult<()> {
